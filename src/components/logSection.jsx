...
export default function LogSection({musicData}) {
  ...
  function next(){
     {id < musicData.length - 1 ? setId(prev => prev + 1) : setId(0)}
  }

  function prev(){
    {id > 0 ? setId(prev => prev - 1) : setId(musicData.length - 1)}
}
...
function pause(){
  ...
  return (
    <section className="logSection">
      <aside>
        <div>
        <audio src={musicData[id].audio} ref={audioRef}>
        </audio> 
          <img src={img1} alt="currentMusicImg" />
          <div>
            <h4>{musicData[id]?.title}</h4>
            <p>{musicData[id]?.artist}</p>
          </div>
        </div>
        <aside>
